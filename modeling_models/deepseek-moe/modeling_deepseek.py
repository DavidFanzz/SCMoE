...
from transformers.modeling_outputs import BaseModelOutputWithPast, CausalLMOutputWithPast, SequenceClassifierOutputWithPast, MoeCausalLMOutputWithPast, MoeModelOutputWithPast
...
class MoEGate(nn.Module):
    ...
    def forward(self, hidden_states, num_experts_per_tok, routed_tok, dynamic_expert_routing_threshold):
        assert num_experts_per_tok == len(routed_tok)
        ...
        routing_weights, selected_experts = torch.sort(scores, dim=-1, descending=True)
        if len(routed_tok) == 1 and routed_tok[0] == 64:
            ...
            selected_experts_random = torch.randint(0, 64, (selected_experts.shape[0], 1)).to(selected_experts.device)
            selected_experts = torch.cat((selected_experts, selected_experts_random), dim=-1)

        if dynamic_expert_routing_threshold != 1.0:
            ...
            new_topk_weight = torch.zeros_like(routing_weights).to(routing_weights.device)
            for _ in range(len(new_topk_weight)):
                if threshold_indices[_] > 0:
                    new_topk_weight[_][:dim_size - threshold_indices[_] + 1] = routing_weights[_][:dim_size - threshold_indices[_] + 1]
                else:
                    new_topk_weight[_] = routing_weights[_]
            break_tag = 0
            for _ in range(num_experts_per_tok):
                if new_topk_weight[:, _:].sum() == 0:
                    topk_weight = new_topk_weight[:, :_]
                    topk_idx = selected_experts[:, :_]
                    break_tag = 1
                    break
            if break_tag == 0:
                topk_weight = routing_weights
                topk_idx = selected_experts
            topk_weight = new_topk_weight
        else:
            topk_weight = routing_weights[:, routed_tok]
            topk_idx = selected_experts[:, routed_tok]
...
class DeepseekMoE(nn.Module):
    ...
    def forward(self, hidden_states, num_experts_per_tok, routed_tok, dynamic_expert_routing_threshold):
        ...
        topk_idx, topk_weight, aux_loss, scores = self.gate(hidden_states, num_experts_per_tok, routed_tok, dynamic_expert_routing_threshold)
        ...
        if self.training:
            ...
            y = self.moe_infer(hidden_states, flat_topk_idx, topk_weight.view(-1, 1), topk_weight.shape[1]).view(*orig_shape)
        if self.config.n_shared_experts is not None:
            y = y + self.shared_experts(identity)[0]
        return y, scores
    
    @torch.no_grad()
    def moe_infer(self, x, flat_expert_indices, flat_expert_weights, num_experts_per_tok):
        ...
        for i, end_idx in enumerate(tokens_per_expert):
            ...
            expert_out = expert(expert_tokens)[0]
